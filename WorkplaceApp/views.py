...
from . models import studentDataTable, companyDataTable, flyerdata, studentfavorite,studentapply, message, ComNotification, StuNotification
...
def createstuacc(request):
    if request.method == 'POST':
        ...
        if studentDataTable.objects.filter(email=email).exists():
            ...
            createu = studentDataTable.objects.create(firstname=fname,lastname=lname,address=address,email=email,tel=phone,university=uni,degree=degree,linkedin=linkedin,username=username,password=password,profilepic=propic)
            createu.save()
...
def createcomacc(request):
    if request.method == 'POST':
        ...
        if companyDataTable.objects.filter(email=email).exists():
            ...
            createu = companyDataTable.objects.create(companyname=comname,comregno=regnmbr,address=address,email=email,tel=phone,preffield=prefield,linkedin=linkedin,username=username,password=password,profilepic=propic)
            createu.save()
...
def comaddpost(request,comid):
    ...
    if request.method == 'POST':
        ...
        newflyer = flyerdata.objects.create(company=comdb,jobfield=jobfield, jobpost=jobpost, salary=salary, location=location, flyerimage=flyerimg, createtimestamp=dateTime, updatetimestamp=dateTime)
        newflyer.save()
...
def ComNotifications(request, comid):
    ...
    return render(request, "company/comnotification.html",{'comdb':comdb,'newNotifications':newNotifications, 'oldNotifications':oldNotifications})
...
def comchat(request, comid, senderid):
    ...
    if request.method== 'POST':
        ...
        current_datetime = datetime.datetime.now()
        dateTime = current_datetime.replace(microsecond=0)
        newmsg = message.objects.create(msg=msg, student=studb, company=comdb, sender=comdb.companyname, senderID=comdb.comid, Receiver=studb.firstname+' '+studb.lastname, ReceiverID=studb.stuid, MarkAsRead='0', createtimestamp=dateTime )
        newmsg.save()
        return redirect('comchat',comid, senderid )

    return render(request, "company/commsgbody.html", {'comdb':comdb, 'studb':studb, 'chatdb':chatdb} )
...
def stufeed(request,stuid):
    if request.method== 'POST':
        ...
        flyerdb = flyerdata.objects.filter(jobfield__contains=field,location__contains = location, jobpost__contains =position )
        return render(request, "student/stufeed.html",{'studb':studb, 'flyerdb':flyerdb, 'stufavdb':stufavdb, 'stuapply':stuapply} )
    else:
        studb = studentDataTable.objects.get(stuid=stuid)
...
def apply(request, stuid, fid):
    ...
    stuapply.save()
    newMsg = message.objects.create(msg='I want to apply to the job vacancy on '+flyrdb.jobpost+' in '+flyrdb.jobfield+' that you posted at '+flyrdb.updatetimestamp,student=studb, company=flyrdb.company,sender=studb.firstname+' '+studb.lastname, senderID=studb.stuid, Receiver=flyrdb.company.companyname,ReceiverID=flyrdb.company.comid , MarkAsRead='0', createtimestamp=dateTime)
    newMsg.save()
    Comnewnotification = ComNotification.objects.create(notification=studb.firstname+' '+studb.lastname+' want to apply to the job vacancy on '+flyrdb.jobpost+' in '+flyrdb.jobfield+' that you posted at '+flyrdb.updatetimestamp,company=flyrdb.company, createtimestamp=dateTime, MarkAsRead='0')
    Comnewnotification.save()
    Stunewnotification = StuNotification.objects.create(notification= 'You send a apply request for the job vacancy on '+flyrdb.jobpost+' in '+flyrdb.jobfield+' that posted by ' +flyrdb.company.companyname +' at ' + flyrdb.updatetimestamp ,student=studb, createtimestamp=dateTime, MarkAsRead='0')
...
def StuNotifications(request, stuid):
    ...
    return render(request, "student/stunotification.html",{'studb':studb,'newNotifications':newNotifications, 'oldNotifications':oldNotifications})
...
def stuchat(request, stuid, senderid):
    ...
    studb = studentDataTable.objects.get(stuid=stuid)
    chatdb = message.objects.filter(ReceiverID__in=[stuid,senderid], senderID__in=[senderid,stuid])
    if request.method== 'POST':
        msg = request.POST['newmsg']
        current_datetime = datetime.datetime.now()
        dateTime = current_datetime.replace(microsecond=0)
        newmsg = message.objects.create(msg=msg, student=studb, company=comdb, sender=studb.firstname+' '+studb.lastname, senderID=studb.stuid, Receiver=comdb.companyname, ReceiverID=comdb.comid, MarkAsRead='0', createtimestamp=dateTime )
        newmsg.save()
...
def viewStudent(request, stuid, comid):
    comdb = companyDataTable.objects.get(comid=comid)
    studb = studentDataTable.objects.get(stuid=stuid)

    return render(request, "company/viewstudent.html", {'studb': studb, 'comdb': comdb})
